...
export default function StoreGRNAdminApproval() {
    ...
    const handleApproveAdmin = async (row: StoreGRNRow) => {
        if (!row.grn_no) return;

        const confirmed = window.confirm(
            `Approve GRN ${row.grn_no} as Admin?`
        );
        if (!confirmed) return;
        ...
        try {
            await storeGRNApprovalApi.approveByAdmin(row.grn_no);
    ...
    return (
        <div className="w-full p-4 md:p-6 lg:p-8">
            <Heading
                heading="Store GRN – Admin Approval"
                subtext="Approve GRN bills sent by store"
            >
                                    ...
                                    <td className="px-3 py-2 text-center">
                                        {activeTab === "pending" ? (
                                            <Button
                                                ...
                                                onClick={() => handleApproveAdmin(row)}
